...
##
export PS1='\[\e[1;32m\]\u\[\e[m\]@\[\e[1;36m\]\h\[\e[m\]:\[\e[1;34m\]\W\[\e[m\]\[\e[0;37m\]\\$\[\e[m\] '
"
...
# LR:Personalizando o prompt do sistema
##
export PS1='\[\e[1;31m\]\u\[\e[m\]@\[\e[1;36m\]\h\[\e[m\]:\[\e[1;34m\]\W\[\e[m\]\[\e[0;31m\]\\$\[\e[m\] '
"

HumanUsers=$(cut -d: -f1,3 /etc/passwd | egrep ':[0-9]{4}$' | cut -d: -f1)

for user in $HumanUsers; do
    UserDir=$(grep "^$user:" /etc/passwd | awk -F ':' '{print $6}')
    if [ -d "$UserDir" ]; then
